    ...
    public static IReadOnlyList<ValidationCheck> Run(string repoRoot)
    {
        ...
        checks.Add(Evaluate(
            ...
            () =>
            {
                ...
                var claude = PromptBundleBuilder.Prepare(repoRoot, "Review repo doctor output", new PromptAssemblyOptions
                {
                    PersonaId = "reviewer",
                    TargetPath = target,
                    Platform = PromptPlatforms.ClaudeCode
                });
                var opencode = PromptBundleBuilder.Prepare(repoRoot, "Review repo doctor output", new PromptAssemblyOptions
                {
                    PersonaId = "reviewer",
                    TargetPath = target,
                    Platform = PromptPlatforms.OpenCode
                });
                ...
                var factory = PromptBundleBuilder.Prepare(repoRoot, "Implement a safer repo validation summary", new PromptAssemblyOptions
                {
                    PersonaId = "implementer",
                    TargetPath = target,
                ...
                var passed = codex.RenderedPrompt.Platform == PromptPlatforms.CodexCli
                             ...
                             && claude.RenderedPrompt.CompositeText.Contains("ROLE", StringComparison.Ordinal)
                             && opencode.RenderedPrompt.CompositeText.Contains("PERSONA", StringComparison.Ordinal)
                             ...
                             && factory.RenderedPrompt.CompositeText.Contains("RUNTIME PROFILE", StringComparison.Ordinal);
                var evidence = $"codex-platform={codex.RenderedPrompt.Platform}; codex-messages={codex.RenderedPrompt.Messages.Count}; claude-platform={claude.RenderedPrompt.Platform}; opencode-platform={opencode.RenderedPrompt.Platform}; copilot-platform={copilot.RenderedPrompt.Platform}; gemini-platform={gemini.RenderedPrompt.Platform}; antigravity-platform={antigravity.RenderedPrompt.Platform}; factory-platform={factory.RenderedPrompt.Platform}";
